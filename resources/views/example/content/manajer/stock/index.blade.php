                ...
                <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="#"
                                class="ml-1 text-gray-700 hover:text-primary-600 md:ml-2 dark:text-gray-300 dark:hover:text-white">Stok Manajemen</a>
                        </div>
                    </li>
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="ml-1 text-gray-400 md:ml-2 dark:text-gray-500">Transaksi Stok</span>
                        </div>
                    </li>
                </ol>
            </nav>
            <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Daftar Transaksi Stok</h1>
        </div>
    </div>
    <div class="px-4 pt-6">
        ...
        <div class="mb-4">
            <a href="{{ route('Manajer.stock.tambah') }}"
                ...
                <tbody>
                    @forelse ($stocks as $key => $stock)
                        <tr
                            ...
                            {{-- <td class="px-6 py-4">
                                Rp. {{ number_format($stock->product->purchase_price, 0, ',', '.') ?? 'N/A' }}
                            </td>
                            <td class="px-6 py-4">
                                Rp. {{ number_format($stock->product->selling_price, 0, ',', '.') ?? 'N/A' }}
                            </td> --}}
                            @if ($stock->type == 'Keluar')
                                <td class="p-4 whitespace-nowrap">
                                    <span
                                        class="bg-red-100 text-red-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded-md border border-red-100 dark:border-red-400 dark:bg-gray-700 dark:text-red-400">{{ $stock->type }}</span>
                                </td>
                            @else
                                <td class="p-4 whitespace-nowrap">
                                    <span
                                        class="bg-green-100 text-green-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded-md dark:bg-gray-700 dark:text-green-400 border border-green-100 dark:border-green-500">{{ $stock->type }}</span>
                                </td>
                            @endif
                            <td class="px-6 py-4 ">
                                {{ $stock->quantity }}
                            ...
                            <td class="px-6 py-4">
                                Rp. {{ number_format($stock->product->selling_price * $stock->quantity, 0, ',', '.') }}
                            </td>

                            <td class="p-4 space-x-2 whitespace-nowrap">
                                <a href="{{ route('Manajer.stock.edit', $stock->id) }}">
                                ...
                                <form action="{{ route('stock.hapus', $stock->id) }}" method="POST"
