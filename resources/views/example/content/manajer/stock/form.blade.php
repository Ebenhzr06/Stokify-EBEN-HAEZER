...
@section('content')
    <div class="px-4 pt-6">
        ...
        <form action="{{ isset($stock) ? route('Manajer.stock.update', $stock->id) : route('Manajer.stock.tambah.simpan') }}"
            method="POST">
            @csrf
            @isset($stock)
                @method('PUT')
            ...
            <div class="space-y-4">
                {{-- Field: No Transaksi (transaction_no) --}}
                {{-- Catatan: Kolom 'transaction_no' belum ada di Model $fillable dan Migrasi Anda saat ini.
                     Jika Anda ingin menggunakannya, Anda harus menambahkan di Model dan Migrasi,
                     lalu hapus komentar di bawah ini. --}}
                {{--
                <div>
                    <label for="transaction_no" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">No. Transaksi</label>
                    <input type="text" name="transaction_no" id="transaction_no"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                        placeholder="TRX-001" value="{{ old('transaction_no', $stock->transaction_no ?? '') }}">
                ...
                {{-- Field: Tanggal Data Dibuat (date) --}}
                <div>
