...
@section('content')
    <div class="px-4 pt-6">
        ...
        <form action="{{ isset($user) ? route('users.update', $user->id) : route('users.store') }}"
            method="POST">
            @csrf
            {{-- Jika ini form edit, gunakan metode PUT --}}
            @isset($user)
                @method('PUT')
            @endisset
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="name" id="name"
                        ...
                        placeholder="Nama Pengguna" required="" value="{{ old('name', $user->name ?? '') }}">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                        placeholder="email@example.com" required="" value="{{ old('email', $user->email ?? '') }}">
                    @error('email')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="password" name="password" id="password"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                        placeholder="********" {{ isset($user) ? '' : 'required' }}> {{-- Password wajib saat buat, opsional saat edit --}}
                    @error('password')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="bottom-0 left-0 flex w-full p-4 space-x-2 whitespace-nowrap">
                    ...
                    <a href="{{ route('users.index') }}"><button type="button" class="inline-flex px-3 py-2 text-sm font-medium text-center text-white bg-red-700 rounded-lg hover:bg-red-800 focus:ring-4 focus:ring-red-300 dark:focus:ring-red-900">
                        Batal
                    </button></a>
                </div>
            </div>
